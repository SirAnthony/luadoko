...
#include "generated/CUDataBind.h"
...
{
	lua_State* Lst = LuaRoutines::routines_state();
	...
	ud = new(ud) CUData(p);
	LuaRoutines::registry_get( Lst, UD_META_REF(T) );  // st: ud meta
	lua_setmetatable( Lst, -2 );                        // st: ud
	ud->setRegRef( LuaRoutines::registry_add() );        // st:
	return ud;
}
...
CUData* check_userdata(lua_State* L, int ud, bool throw_error /* = true */ )
{
	...
	if( p != NULL ){  /* value is a userdata? */
		if( lua_getmetatable( L, ud ) ){  /* does it have a metatable? */
			LuaRoutines::registry_get( L, UD_META_REF(T) );  /* get correct metatable */
...
#include "CUDataMacro.h"
#include "CUDataMacroExt.h"
...
void RegisterTypeMetatable(lua_State* L)
{
	...
	UD_META_REF(T) = LuaRoutines::registry_add();
	// st:
}
